...
def toggle_usb_power(turn_on=True):
    ...
    try:
        ...
        # Try using direct sysfs control - this may affect all USB devices
        if turn_on:
            # Try to bind USB devices
            subprocess.run(['sudo', 'sh', '-c', 'echo "1-1" > /sys/bus/usb/drivers/usb/bind'], check=False)
            print("Attempted to bind USB devices")
            return True
        else:
            # Try to unbind USB devices
            subprocess.run(['sudo', 'sh', '-c', 'echo "1-1" > /sys/bus/usb/drivers/usb/unbind'], check=False)
            print("Attempted to unbind USB devices")
            return True
