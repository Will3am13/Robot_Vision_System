...
from camera_manager_with_script import CameraManager

# Configuration paths
OAK_D_SR_MODEL_PATH = r"/home/er/Downloads/BatteryV2.blob"
MINI_CAM_MODEL_PATH = r"/path/to/your/top_battery.pt"  # Update with actual path
MINI_CAM_ID = 1

# Configure conversion factors based on your system calibration
X_PIXEL_TO_MM = 0.1  # Conversion factor from x-axis pixels to mm (adjust based on calibration)
Y_PIXEL_TO_MM = 0.1  # Conversion factor from y-axis pixels to mm (adjust based on calibration)
ANGLE_TO_ROBOT = 1.0  # Conversion factor from detected angle to robot angle

# Define a standby (home) position - from original code
STANDBY_COORDS = [111.7, -53.4, 251.2, 172.67, -9.25, 46.06]
STANDBY_ANGLES = [0.52, 23.9, -75.93, -18.45, 0.08, -131.48]

# Bin locations (fixed positions for sorting) - from original code
...
def pick_and_place_battery(mc, cam_manager, camera_coords, is_cbattery=False):
    ...
    # STEP 1: Pause Oak-D camera processing while robot is moving
    cam_manager.set_camera_active(False)

    # STEP 2: Move to hover position above the target
    print(f"Moving to hover position {hover_coords} (60 units above target)")
    mc.send_coords(hover_coords, 30, 1)
    time.sleep(2)

    # STEP 3: Use mini camera for fine-tuning position
    print("\nActivating mini camera for fine position adjustment...")
    success, adjusted_xyz, adjusted_angle = cam_manager.use_mini_cam_for_fine_tuning(
        mc=mc,
        current_xyz=hover_xyz,
        threshold_distance=10,  # 10 pixels threshold from center
        threshold_angle=5,  # 5 degrees threshold from 90°
        max_attempts=3,  # Maximum 3 adjustment attempts
        visualization=False  # Don't show camera view to save resources
    )

    if success:
        print("Position successfully fine-tuned using mini camera.")

        # Update hover and pick coordinates with adjusted values
        hover_xyz = adjusted_xyz
        hover_coords = list(hover_xyz) + [180, 0, adjusted_angle]

        # Update pick coordinates
        pick_xyz = hover_xyz.copy()
        pick_xyz[2] -= 60  # Move down from hover position
        pick_coords = list(pick_xyz) + [180, 0, adjusted_angle]
    else:
        print("Could not fine-tune position with mini camera. Continuing with original coordinates.")

    # STEP 4: Move down to the actual target position
    ...
    # Re-activate Oak-D camera after returning to standby
    print("Re-activating Oak-D-SR camera for detection...")
    cam_manager.set_camera_active(True)
...
def main():
    ...
    # Initialize camera manager but don't start camera processing yet
    cam_manager = CameraManager(
        oak_d_model_path=OAK_D_SR_MODEL_PATH,
        mini_cam_model_path=MINI_CAM_MODEL_PATH,
        mini_cam_id=MINI_CAM_ID,
        oak_camera_active=False  # Start with camera inactive to save resources
    )

    # Start the Oak-D-SR device (connects to device but doesn't process frames yet)
    cam_manager.start_device()

    print("System initialized. Commands:")
    print("  'p' - pick up detected battery")
    print("  'a' - toggle auto-mode")
    print("  'c' - toggle Oak-D camera processing")
    print("  'q' - quit")
    ...
    # Start with camera turned off - will be turned on when at standby
    auto_mode = False
    last_processed_time = 0
    cooldown_time = 10  # Seconds between auto-processing

    # Activate camera processing when robot is at standby
    cam_manager.set_camera_active(True)
    ...
    while True:
        # Create a blank frame when camera is inactive
        frame = np.zeros((480, 640, 3), dtype=np.uint8)
        height, width = frame.shape[:2]

        # Display camera status
        status_text = f"Oak-D Camera: {'ACTIVE' if cam_manager.oak_camera_active else 'PAUSED'}"
        status_color = (0, 255, 0) if cam_manager.oak_camera_active else (0, 0, 255)
        cv2.putText(frame, status_text, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, status_color, 2)

        # Display auto mode status
        auto_text = f"Auto Mode: {'ON' if auto_mode else 'OFF'}"
        auto_color = (0, 255, 0) if auto_mode else (0, 0, 255)
        cv2.putText(frame, auto_text, (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, auto_color, 2)

        # Display controls
        cv2.putText(frame, "p: pick  a: toggle auto  c: toggle camera  q: quit", (10, height - 20),
        ...
        # Process frame only if Oak-D camera is active
        if cam_manager.oak_camera_active:
            # Get frames and detections
            new_frame, detections = cam_manager.get_frame()
            ...
            if new_frame is not None:
                ...
                for detection in detections:
                    ...
                    if class_name not in class_settings or detection.confidence < class_settings[class_name][
                        "threshold"]:
                        continue

                    # Get bounding box coordinates
                    xmin, ymin = int(detection.xmin * width), int(detection.ymin * height)
                    xmax, ymax = int(detection.xmax * width), int(detection.ymax * height)
                    ...
                    # Use the original_distance directly
        ...
        if key == ord('c'):
            cam_manager.toggle_camera_active()
            print(f"Oak-D camera processing {'activated' if cam_manager.oak_camera_active else 'paused'}")
